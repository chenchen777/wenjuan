 ...
 */
class JMonitorClockController extends Controller
{
    ...
    private function getKeywordList(Connection $redis, $isApp = true){
        ...
        $condition = [
            'a.user_id' => User::getActiveUserIds(),
            'a.is_category' => 0,
            'a.is_update_'.$type => 0,
            'a.monitor_close' => 0,
            'a.deleted' => 0,
        ...
        foreach ($keywordQuery->each() as $keyword_id => $each){
            echo $keyword_id . $each->keyword . PHP_EOL;
            $redis->sadd($idAll, $keyword_id);
        }

        //求并集：最多查询2次的关键词
        $redis->sunionstore($type . '_ids_twice', $idAll, $type . '_success_ids');
        // 求交集：最多查询2次的关键词
...
/*************************************APP端关键词排名每日更新脚本-start***********************************/
    ...
    private function rankAppDaily(Connection $redis, MonitorKeyword $mKeyword, MonitorGoods $mGoods, $flag){
        ...
        echo 'search_type=' . $search_type . PHP_EOL;
        // 获取pc端和app端接口查询所需参数
    ...
    private function appSaveResult(Connection $redis, MonitorKeywordResult $model, MonitorKeyword $mKeyword, $appMatch){
        ...
        try{
            ...
            if (!empty($nowShow) &&
                ...
                echo 'keyword_id:' . $mKeyword->id . 'new_page_order_app:' . intval($nowShow->page_order_app) . 'old_page_order_app:' . intval($model->page_order_app) . PHP_EOL;
        ...
        for ($fork_id = 1; $fork_id <= $forkNum; $fork_id++){
            ...
            if ($pid == -1) { // 进程创建失败
                ...
                while (1){
                    ...
                    echo 'keyword_id:' . $keyword_id . PHP_EOL;
                    if (!$keyword_id || $keyword_id == 'nil'){
                        break;
                    }
                    system('cd /data/wwwroot/jzhentan/jdcha && php ./yii j-monitor-clock/fork-update-' . $type . ' ' . $keyword_id . ' ' . 'twice');
                }
...
//                        break;
//                    }
//                    system('cd /data/wwwroot/jzhentan/jdcha && php ./yii j-monitor-clock/fork-update-' . $type . ' ' . $keyword_id . ' ' . 'once');
//                }
...
//                unset($redisKey);
                ...
                while (1){
                    ...
                    system('cd /data/wwwroot/jzhentan/jdcha && php ./yii j-monitor-clock/fork-update-' . $type . ' ' . $keyword_id . ' ' . 'redo');
                }
        ...
        do{
            ...
            foreach ($cmds as $cmd) {
                ...
                if ($pid == - 1) { // 进程创建失败
                    ...
                    // 子进程处理逻辑
                    foreach ($cmd as $line) {
                        system('cd /data/wwwroot/jdcha && ' . 'php ' . './yii j-monitor-clock/aaaa ' . $line);
                        echo $line . " done!\n";
                    }
                    exit(0);
                }
            ...
            try{
                ...
                foreach ($cmds as $cmd) {
                    ...
                    if ($pid == - 1) { // 进程创建失败
                        ...
                        // 子进程处理逻辑
                        foreach ($cmd as $line) {
                            system('cd /data/wwwroot/jdcha && ' . 'php ' . './yii j-monitor-clock/key-update ' . $line);
                            echo $line . " done!\n";
                        }
                        exit(0);
                    }
